class MailChimpCrawler
  ...
  def all_campaigns
    response = HTTParty.get("https://us5.api.mailchimp.com/2.0/campaigns/list?apikey=#{ENV['API_KEY']}&limit=10&filters[status]=sent&filters[sendtime_end]=#{3.days.ago.strftime("%F")}")
    response["data"]
  end
